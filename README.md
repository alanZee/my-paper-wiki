...
面向学术文献管理的 **LLM Wiki** skill 包。基于 Karpathy LLM Wiki 的持久、增量、可复用理念，针对学术场景做了专门适配：以论文为原子单元，强制引用审计门禁，保证知识页面的学术规范性与可追溯性。

本 skill 覆盖文献知识库从**构建到维护**的完整生命周期：

- **知识库构建**：从论文原文件（PDF/Tex）自动提取结构化信息，生成深度 wiki 页面（问题定义、方法细节、量化结果、关键贡献等）
- **知识查询**：基于已有 stable 论文回答研究问题，支持按 topic 汇聚相关论文页面
- **学术综述**：自动聚合同主题 stable 论文，生成综述草稿并导出 LaTeX（使用 `\cite{bibkey}` 引用）
- **质量门禁**：对每篇论文页面执行引用支撑审计（Phase A/B），未通过审计的页面不得作为综述证据源
- **持续维护**：支持增量添加新论文、修订已有页面（diff-before-write）、一致性健康检查

以 skill 形态交付，跨 Claude Code / OpenCode / Codex / Cursor 等主流 AI 编程 harness 通用。
